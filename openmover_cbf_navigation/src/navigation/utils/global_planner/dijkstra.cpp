...
#define AVOID_OBSTACLE // Improved Dijkstra
...
inline void DijkstraExpansion::updateCell(unsigned char *costs, float *potential, int n)
{
    ...
    // 针对不同的机器人及场景，distanceMultiplier与Weight需要调整
    float Weight1 = 4.0;
    float distanceMultiplier = 1;
    float Weight2 = 1.45;
    float obstacleDistance_n = shortest_distances_[n];

    c = Weight1 * c + Weight2 * c * exp(-distanceMultiplier * obstacleDistance_n);
    ...
    {
#ifdef AVOID_OBSTACLE
        float obstacleDistance_n1 = shortest_distances_[n - 1];
        float obstacleDistance_n2 = shortest_distances_[n + 1];
        float obstacleDistance_n3 = shortest_distances_[n - nx_];
        float obstacleDistance_n4 = shortest_distances_[n + nx_];

        float le = INVSQRT2 * (float)(Weight1 * getCost(costs, n - 1) + Weight2 * getCost(costs, n - 1) * exp(-distanceMultiplier * obstacleDistance_n1));
        float re = INVSQRT2 * (float)(Weight1 * getCost(costs, n + 1) + Weight2 * getCost(costs, n + 1) * exp(-distanceMultiplier * obstacleDistance_n2));
        float ue = INVSQRT2 * (float)(Weight1 * getCost(costs, n - nx_) + Weight2 * getCost(costs, n - nx_) * exp(-distanceMultiplier * obstacleDistance_n3));
        float de = INVSQRT2 * (float)(Weight1 * getCost(costs, n + nx_) + Weight2 * getCost(costs, n + nx_) * exp(-distanceMultiplier * obstacleDistance_n4));
#else
